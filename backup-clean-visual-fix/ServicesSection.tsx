...
import { ArrowUpRight, Building2, Home, Laptop, Plane, Refrigerator, Sparkles } from "lucide-react";
...
export default function ServicesSection() {
  return (
    <section id="services" className="relative overflow-hidden bg-[#0F172A] py-20 sm:py-24">
      <div className="absolute right-[-8%] top-20 hidden h-[520px] w-[55%] opacity-25 lg:block">
        <Image src="/visual-services.png" alt="" fill sizes="50vw" className="object-contain" />
      </div>
      <div className="relative mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
        <div className="max-w-3xl">
          <p className="text-sm font-black uppercase tracking-[0.2em] text-violet-400">Our Services</p>
          <h2 className="mt-4 text-4xl font-black tracking-tight text-white sm:text-5xl">Solutions that make life easier.</h2>
          <p className="mt-5 max-w-2xl leading-7 text-slate-400">Members register their interest first. Available packages become visible in the member portal after approval.</p>
        </div>
        <div className="mt-14 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {services.map(([title,description,href,Icon,status]) => (
            <Link key={title} href={href} className="group rounded-[1.65rem] border border-white/10 bg-[#111C35]/90 p-6 backdrop-blur transition hover:-translate-y-2 hover:border-violet-400/40">
              <div className="flex items-center justify-between">
                <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-[#6D3BFF] to-[#172B63] text-white"><Icon className="h-6 w-6" /></div>
                <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[10px] font-black uppercase tracking-wider text-slate-400">{status}</span>
              </div>
