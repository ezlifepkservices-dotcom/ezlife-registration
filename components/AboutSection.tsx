...
import { ArrowRight, HeartHandshake, Scale, ShieldCheck, UsersRound } from "lucide-react";
  ...
  return (
    <section id="about" className="relative overflow-hidden bg-[#0B1222] py-20 sm:py-24">
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-5 sm:px-6 lg:grid-cols-[0.92fr_1.08fr] lg:px-8">
        <div>
          <p className="text-sm font-black uppercase tracking-[0.2em] text-violet-400">About EZ Life</p>
          <h2 className="mt-4 text-4xl font-black leading-tight text-white sm:text-5xl">
            A structured platform designed to
            <span className="block text-violet-300">serve members better.</span>
          </h2>
          <p className="mt-5 max-w-xl leading-8 text-slate-400">
            EZ Life connects approved members with selected service packages through registration, verification, package selection, payment tracking and transparent progress monitoring.
          </p>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {values.map(([title, description, Icon]) => (
            <article key={title} className="rounded-2xl border border-white/10 bg-white/[0.045] p-5 backdrop-blur">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-violet-500/15 text-violet-300"><Icon className="h-5 w-5" /></div>
              <h3 className="mt-4 font-black text-white">{title}</h3>
              <p className="mt-2 text-sm leading-6 text-slate-400">{description}</p>
            </article>
          ))}
          <div className="relative min-h-[320px] overflow-hidden rounded-2xl border border-white/10 sm:col-span-2">
            <Image src="/visual-about.jpg" alt="Umrah opportunity" fill sizes="(max-width:1024px) 100vw,700px" className="object-cover" />
